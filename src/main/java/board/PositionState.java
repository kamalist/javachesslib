...
public class PositionState {
	...
	private static HashMap<Piece,List<String>> deepCopyHashMap(HashMap<Piece,List<String>> oldMap) {
		...
		for(int i=0;i<arr.length;i++) {
			...
			for(int j=0;j<ct;j++) {
				newlist.add(new String(oldlist.get(j)));
			}
	...
	/** Generates pseudo-legal moves as if a bishop was on <tt>startingSquare</tt>. */
	protected static String[] generatePseudoLegalBishopMoves(String startingSquare) {
	...
	/** Generates pseudo-legal moves as if a king was on <tt>startingSquare</tt>. */
	protected static String[] generatePseudoLegalKingMoves(String startingSquare) {
		return getAdjacentSquares(startingSquare);
	}
	
	/** Generates pseudo-legal moves as if a knight was on <tt>startingSquare</tt>. */
	protected static String[] generatePseudoLegalKnightMoves(String startingSquare) {
		List<String> list = new ArrayList<String>();
		
		int sq = squareTo0x88(startingSquare);
		int[] directions = new int[] {
				-0x21, 0x21, 0x12, -0x12, 0xE, -0xE, 0x1F, -0x1F
		};
		for(int i=0;i<directions.length;i++) {
			int newsq = sq+directions[i];
			if ((newsq & 0x88) == 0) {
				list.add(squareFrom0x88(newsq));
			}
		}
	...
	/** Generates pseudo-legal moves as if a queen was on <tt>startingSquare</tt>. */
	protected static String[] generatePseudoLegalQueenMoves(String startingSquare) {
		String[] bishop = generatePseudoLegalBishopMoves(startingSquare);
		String[] rook = generatePseudoLegalRookMoves(startingSquare);
	...
	/** Generates pseudo-legal moves as if a rook was on <tt>startingSquare</tt>. */
	protected static String[] generatePseudoLegalRookMoves(String startingSquare) {
		List<String> list = new ArrayList<String>();
		
		final int sq = squareTo0x88(startingSquare);
		
		int mysq = sq;
		int[] directions = { -0x1, 0x1, -0x10, 0x10 };
		for(int i=0;i<directions.length;i++) {
			mysq = sq;
			while((mysq+directions[i] & 0x88) == 0) {
				mysq += directions[i];
				list.add(squareFrom0x88(mysq));
			}
		}
	...
	/** Generates pseudo-legal moves as if a king was on <tt>startingSquare</tt>. */
	protected static String[] generatePsuedoLegalKingMoves(String startingSquare) {
		return getAdjacentSquares(startingSquare);
	}
	
	/** Returns adjacent squares in every direction.<br />
	...
	public static String[] getAdjacentSquares(String square) {
		...
		int sq = squareTo0x88(square);
		int[] directions = new int[] { -0x11, -0x01, 0xF, -0xF, 0x01, 0x11,
				-0x10, 0x10 };
		for (int i = 0; i < directions.length; i++) {
			int newsq = sq + directions[i];
			if (isValidSquare(newsq)) {
				list.add(squareFrom0x88(newsq));
			}
		}
	...
	public static int getDirectionBetween(String from, String to) {
	...
	public static PositionState getStartingPositionState() {
		...
		ps.setPrinter(DefaultPrinter.getSingletonInstance());
	...
	public static boolean isDiagonal(String from,String to) {
		int direction = getDirectionBetween(from, to);
		return direction == 0xF || direction == -0xF || direction == 0x11 || direction == -0x11;
	}
	
	/** Even if a king can't go to that square, it can still be attacking it. */
	public static boolean isKingAttackingSquare(String from,String to) {
		return isAdjacentSquare(from, to);
	}
	
	public static boolean isSameFile(String from,String to) {
		int direction = getDirectionBetween(from, to);
		return direction == 0xF || direction == -0xF;
	}
	
	public static boolean isSameRank(String from,String to) {
		int direction = getDirectionBetween(from, to);
	 ...
	 * </code>
	 * */
	public static boolean isSameRankOrFile(String from,String to) {
	...
	protected static String[] mergeArrays(String[][] arr) {
		List<String> list = new ArrayList<String>();
		for(int i=0;i<arr.length;i++) {
	...
	public String[] generateLegalBishopMoves(boolean white) {
		...
		for(String fromSquare : squares) {
			String[] arr = generatePseudoLegalBishopMoves(fromSquare);
	...
	public String[] generateLegalKnightMoves(boolean white) {
		...
		if (squares == null) return null;
		for(String fromSquare : squares) {
			String[] arr = generatePseudoLegalKnightMoves(fromSquare);
			for(String sq : arr) {
				Piece p = getPiece(sq);
				//if (p == Piece.EMPTY) continue;
				if (p != Piece.EMPTY && p.isWhite() == white) continue;
	...
	public String[] generateLegalQueenMoves(boolean white) {
		...
		if (squares == null) return null;
		for(String fromSquare : squares) {
			String[] arr = generatePseudoLegalQueenMoves(fromSquare);
	...
	public String[] generateLegalRookMoves(boolean white) {
		...
		if (squares == null) return null;
		for(String fromSquare : squares) {
			String[] arr = generatePseudoLegalRookMoves(fromSquare);
			int lastDirectionBlocked = -1;
			for(String sq : arr) {
				if (lastDirectionBlocked == getDirectionBetween(fromSquare, sq)) continue;
				Piece p = getPiece(sq);
	...
	public String[] generatePsuedoLegalMoves(boolean white) {
		...
		if (knights != null) {
			for(String s : knights) {
				String[] arr = generatePseudoLegalKnightMoves(s);
		...
		if (bishops != null) {
			for(String s : bishops) {
				String[] arr = generatePseudoLegalBishopMoves(s);
				//System.out.println(s + " " + java.util.Arrays.deepToString(arr));
				for(int i=0;i<arr.length;i++) {
					fromSquares.add((s + arr[i]).toLowerCase());
				}
		...
		if (rooks != null) {
			for(String s : rooks) {
				String[] arr = generatePseudoLegalRookMoves(s);
				//System.out.println(s + " " + java.util.Arrays.deepToString(arr));
				for(int i=0;i<arr.length;i++) {
					fromSquares.add((s + arr[i]).toLowerCase());
				}
		...
		if (queens != null) {
			for(String s : queens) {
				String[] arr = generatePseudoLegalQueenMoves(s);
				//System.out.println(s + " " + java.util.Arrays.deepToString(arr));
				for(int i=0;i<arr.length;i++) {
					fromSquares.add((s + arr[i]).toLowerCase());
				}
	...
	protected int getColorBoardStrength(boolean white) {
		...
		final int[] strengthArr = new int[] { 1,3,3,5,0,9 };
  ...
  board file  (numbered 0--7 for a--h) in which the double push was made */
	...
	public boolean isLegalKingMove(String from,String to) {
		...
		// will this code work instead?
		
		/*int direction = getDirectionBetween(from, to);
		String[] squaresBetween = getSquaresBetween(from, to);
		if (direction == 1 && squaresBetween.length==2) {
			// king side castling
		} else if (direction == -1 && squaresBetween.length==3) {
			// queen side castling
		}
		 */
...
//			if (canBlackCastleQueenside()) return true;
	...
	public boolean isLegalKnightMove(String from,String to) {
		final int fromSq = squareTo0x88(from);
		final int toSq = squareTo0x88(to);
		...
		for(int i=0;i<directions.length;i++) {
			final int newsq = fromSq+directions[i];
			if (toSq == newsq) return true;
		}
	...
	public boolean isPawnAttackingSquare(boolean white,String from,String to) {
		if (!isAdjacentSquare(from, to)) return false;
		
		int direction = getDirectionBetween(from, to);
		System.out.println(direction);
		if (white && (direction == 0xF || direction == 0x11)) {
			return true;
		} else if (!white && (direction == -0xF || direction == -0x11)) {
			return true;
		}
		return false;
	}
